...
#define MAX_SKID_TRACK_SPEED    700.0f
...
#define MAX_REAR_SIDE_SLIP      0.6f
#define MAX_FRONT_SIDE_SLIP     2.0f

using namespace Offroad;

NewVehicle::NewVehicle(Context* context)
    ...
    , steering_( 0.0f )
{
    ...
    m_fmaxEngineForce = 450.f;
    m_fmaxBreakingForce = 200.f;
    ...
    m_fwheelRadius = 0.4f;
    m_fwheelWidth = 0.4f;
    m_fFrontwheelFriction = 3.0f;
    m_fRearwheelFriction = 2.0f;
    ...
    SubscribeToEvent(E_VEHICLE_SPEED, URHO3D_HANDLER(NewVehicle, HandleSpeedChange));
}
...
void NewVehicle::RegisterObject(Context* context)
{
    context->RegisterFactory<NewVehicle>();
...
void NewVehicle::ApplyAttributes()
{
}

void NewVehicle::Init()
{
    ResourceCache* cache = GetSubsystem<ResourceCache>();
   
    SubscribeToEvent(node_, E_NODECOLLISIONSTART, URHO3D_HANDLER(NewVehicle, HandleNodeCollision));

    /*NetworkPriority* np = node_->CreateComponent<NetworkPriority>();
    ...
    Model *vehModel = cache->GetResource<Model>("Offroad/Models/car.mdl");
    hullObject->SetModel(vehModel);
    int random = Random(6);
    if (random == 0) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/Red.xml"));
    }
    else if (random == 1) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/Green.xml"));
    }
    else if (random == 2) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/Blue.xml"));
    }
    else if (random == 3) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/Yellow.xml"));
    }
    else if (random == 4) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/Pink.xml"));
    }
    else if (random == 5) {
        hullObject->SetMaterial(cache->GetResource<Material>("Materials/Colors/turquoise.xml"));
    }
   
    hullObject->SetCastShadows(true);

    // set convex hull and resize local AABB.Y size
    Model *vehColModel = cache->GetResource<Model>("Offroad/Models/car.mdl");
    ...
    for ( int i = 0; i < numWheels_; i++ )
    {
        ...
        if (i < 2)
        {
            wheel.m_sideFrictionStiffness = MAX_FRONT_SIDE_SLIP;
            wheel.m_frictionSlip = m_fFrontwheelFriction;
        }
        else
        {
            m_fRearSlip = MAX_REAR_SIDE_SLIP;
            wheel.m_sideFrictionStiffness = MAX_REAR_SIDE_SLIP;
            wheel.m_frictionSlip = m_fRearwheelFriction;
        }
    }
    ...
    if ( raycastVehicle_ )
    {
		...
		const Color LtWhite(0.8f, 0.8f, 0.8f, 0.4f);
        ...
        for ( int i = 0; i < raycastVehicle_->GetNumWheels(); i++ )
        {
            ...
            wheelNode->SetWorldPosition( v3Origin );
            btWheelInfo &whInfo = raycastVehicle_->GetWheelInfo( i );
            Vector3 v3PosLS = ToVector3( whInfo.m_chassisConnectionPointCS );

            wheelNode->SetWorldRotation( v3PosLS.x_ >= 0.0 ? Quaternion(0.0f, 0.0f, -90.0f) : Quaternion(0.0f, 0.0f, 90.0f) );
            ...
            // particle emitter
            Node *pNodeEmitter = node_->CreateChild();
            Vector3 emitPos = v3Origin + Vector3(0,-m_fwheelRadius,0);
            pNodeEmitter->SetWorldPosition( emitPos );
            NetworkPriority* np = node_->CreateComponent<NetworkPriority>();
            np->SetDistanceFactor(1.0f);
            np->SetBasePriority(50);
            ParticleEmitter* particleEmitter = pNodeEmitter->CreateComponent<ParticleEmitter>();
            particleEmitter->SetEffect( cache->GetResource<ParticleEffect>("Offroad/Particles/Snow.xml"));
    ...
    engineSoundSrc_->SetDistanceAttenuation( 1.0f, 200.0f, 2.0f );
    engineSoundSrc_->SetSoundType(SOUND_EFFECT);
    engineSoundSrc_->SetGain(0.1f);
    engineSoundSrc_->Play(engineSnd_);
    engineSoundSrc_->SetFrequency(AUDIO_FIXED_FREQ_44K * 0.01f);
    ...
    skidSoundSrc_->SetDistanceAttenuation( 1.0f, 200.0f, 1.0f);
    ...
    shockSoundSrc_->SetDistanceAttenuation( 1.0f, 200.0f, 1.0f);

    // acceleration sound while in air - most probably want this on
    playAccelerationSoundInAir_ = false;
}

void NewVehicle::FixedUpdate(float timeStep)
{
    if (!raycastVehicle_->IsActive()) {
        //Disable sleeping for rigid body
...
void NewVehicle::FixedPostUpdate(float timeStep)
{
...
void NewVehicle::PostUpdate(float timeStep)
{
    float curSpdMph = GetSpeedMPH();
...
void NewVehicle::UpdateSteering(float newSteering)
{
    // gradual change
    if ( newSteering != 0.0f )
    {
...
void NewVehicle::ApplyEngineForces(float accelerator, bool braking)
{
    // 4x wheel drive
...
bool NewVehicle::ApplyStiction(float steering, float acceleration, bool braking)
{
...
void NewVehicle::ApplyDownwardForce()
{
...
void NewVehicle::AutoCorrectPitchRoll()
{
...
void NewVehicle::LimitLinearAndAngularVelocity()
{
    // velocity limit
    Vector3 linVel = raycastVehicle_->GetLinearVelocity();
...
void NewVehicle::PostUpdateSound(float timeStep)
{
    int playSkidSound = 0;
...
void NewVehicle::PostUpdateWheelEffects()
{
    float curSpdMph = GetSpeedMPH();
    Vector3 linVel = raycastVehicle_->GetLinearVelocity();
    ...
    for ( int i = 0; i < raycastVehicle_->GetNumWheels(); ++i )
    {
        ...
        if ( whInfo.m_raycastInfo.m_isInContact && whInfo.m_skidInfoCumulative < 2.9f )
        {
            Vector3 pos2 = ToVector3(whInfo.m_raycastInfo.m_contactPointWS);
            particleEmitterNodeList_[i]->SetWorldPosition(pos2);
...
void NewVehicle::DebugDraw(const Color &color)
{
...
void NewVehicle::setReady(bool val)
{
	//GetNode()->SetEnabled(val);
	raycastVehicle_->SetEnabled(val);
}

void NewVehicle::setVehicleID(unsigned int id)
{
	_id = id;
}

unsigned int NewVehicle::getVehicleID()
{
	return _id;
}

bool NewVehicle::isReady()
{
	return raycastVehicle_->IsEnabled();
}


Node* NewVehicle::getNode()
{
	return GetNode();
}

Component* NewVehicle::getComponent()
{
    return this;
}

void NewVehicle::HandleSpeedChange(StringHash eventType, VariantMap& eventData)
{
    int val = eventData[P_VALUE].GetInt();
    m_fmaxEngineForce = val;
}

void NewVehicle::sendPredictedNodeIds()
{
    for (auto it = m_vpNodeWheel.Begin(); it != m_vpNodeWheel.End(); ++it) {
        VariantMap map;
        map[P_NODE_ID] = (*it)->GetID();
...
void NewVehicle::HandleNodeCollision(StringHash eventType, VariantMap& eventData)
{
    ...
    if (!node_->GetChild("CollisionSound")) {
        ...
        Sound* sound;
        if (random == 0) {
            sound = cache->GetResource<Sound>("Sounds/Metal-Metal.wav");
        }
        else if (random == 1) {
            sound = cache->GetResource<Sound>("Sounds/Metal-Metal1.wav");
        }
        else if (random == 2) {
            sound = cache->GetResource<Sound>("Sounds/Metal-Metal2.wav");
        }
        else {
            sound = cache->GetResource<Sound>("Sounds/Metal-Metal3.wav");
        }
